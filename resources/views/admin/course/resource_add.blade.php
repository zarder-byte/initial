...
@section('content')
@page_title(['title'=>'资源关联','conment'=>"为 {$chapter->title} - {$course->title} 关联资源"])
    ...
    <div class='col-12'>
        <table class="table table-sm">
            ...
            <form action="{{route('admin.course.resource.add',[$course->id,$chapter->id])}}" method="post" >
                ...
                <tr>
                    <td>
                        <input type="text" class="form-control  form-control-sm" name="resource_id[]" value="{{$resource->id}}">
                    </td>
                    <td>
                        <input type="text" class="form-control  form-control-sm" name="sort[]" value="{{$resource->pivot->sort}}">
                    </td>
                    <td>
                        ({!! $resource->typeName !!})
                ...
                <tr style="background:#f8f8f8">
                    <td>
                        <input type="text" class="form-control form-control-sm" name="resource_id[]" placeholder="目标资源" >
                    </td>
